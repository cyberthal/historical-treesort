...
When you throw text to a file, ts-throw puts the text at the bottom. EXCEPT when the file already has a top-level heading. Then ts-throw assumes this is a polished document, not an inbox file. ts-throw worries you will forget about text appended to polished documents. So it prepends the text before the top-level headline, where it will stick out like a sore thumb.

ts-throw assumes that most headings you file will have four or more stars. Why? Imagine you are throwing headings to an outline. The 1-heading is the document title. The 2-headings are categories. The 3-headings are subcategories. The 4-headings are topics. Outlines become unwieldy when they get too deep, at which point it's better to create more files and directories to spread the load. 

ts-throw only imposes this opinion on you in one way: it creates Inbox.org files with a "**** offset" at the top. You can still file 5-headings, but they might "vanish" if you accidentally file a 4-heading that folds appended 5-headings beneath it. You can also file 3-headings, although they won't be children of the "offset" heading, and might unexpectedly subsume 4-headings. I recommend that you convert headings to 4-star for transport, and then resize them at their destination.
